...
function renderTask({ id, title, description, dueDate, completed }) {
  ...
  */
  ...
  if (completed) {
    ...
    <li class="select-none mt-2 py-2 border-b border-blue-300">
      <div class="flex items-center">
        <h3 class="mb-3 flex-1 text-xl font-bold text-gray-400 uppercase">${title}</h3>
        <div>
          <span>${dueDate}</span>
          <button onclick="deleteTask(${id})" class="inline-block hover:drop-shadow-md bg-green-300 text-xs text-green-900 border border-white px-3 py-1 rounded-md ml-2">Good Job! Ta bort</button>
        </div>
      </div>`;
  } else {
    html = `
      <li class="select-none mt-2 py-2 border-b border-blue-300">
        <div class="flex items-center">
          <h3 class="mb-3 flex-1 text-xl font-bold text-blue-300 uppercase">${title}</h3>
          <div>
            <span>${dueDate}</span>
            <button onclick="deleteTask(${id})" class="inline-block hover:drop-shadow-md bg-gray-300 text-xs text-black-900 border border-white px-3 py-1 rounded-md ml-2">Ta bort</button>
          </div>
        </div>`;
  }
  ...
  html += `
        ...
        <label for="default-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Klar</label>
    </li>`;
  /***********************Labb 2 ***********************/
